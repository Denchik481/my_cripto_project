...
print(
    f"[{datetime.now()}] Отклонено {len(rejected_cols)} колонок по порогам. "
    f"Оставлено: {len(selected_cols)}"
)
if not selected_cols:
    raise ValueError(
        "Ни одной подходящей колонки не осталось после фильтрации по порогам."
    )
...
# ----------------------
print(
    f"[{datetime.now()}] Чтение выбранных колонок и удаление строк с пустыми "
    f"значениями..."
)
df = con.execute(
...
print(
    f"[{datetime.now()}] Удалено {before_drop - after_drop} строк, где все "
    f"значения были пустыми"
)
...
print(
    f"[{datetime.now()}] Сохранено {after_drop} строк и {len(selected_cols)} "
    f"колонок в: {bronze_path}"
)
